...
Next, please `git commit` and then `git push` the root of the new codebase to my GitHub repo `git@github.com:nordeim/event-spark-2.git` using the attached ssh key and wrapper script. refer to the attached `how-to-git-push-using-ssh-wrapper_SKILL.md` for instruction to use ssh wrapper script for `git push`.
...
Finally, `git commit` locally and then `git push` to `git@github.com:nordeim/event-spark-2.git` using the ssh key:
